...
Completes the maturity ladder of :mod:`erhino.radio.sky.projection`
...
the exact transpose that :class:`~erhino.radio.filters.SkySpaceFilter`
map-making requires.
...
the :class:`~erhino.radio.sky.projection.AbstractSkyProjector` seam. It is
imported lazily so e-RHINO's dependencies are unchanged.
...
class NativeLimTODProjector(AbstractSkyProjector):
    ...
    :class:`~erhino.radio.sky.projection.LimTODProjector`):
