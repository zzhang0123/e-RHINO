...
be inferred and ``None`` everywhere else::
    ...
    from dirt.inference import prior_template, set_prior, to_numpyro_model
...
:mod:`dirt.inference.uncertainty`'s summaries).
"""
...
def _require_numpyro():
    try:
        ...
        raise ImportError(
            "This feature needs numpyro: pip install 'dirt[numpyro]'."
        ) from exc
...
def _site_name(pipeline: AbstractOperator, path: tuple) -> str:
    ...
    from dirt.core.graph import Assembly
